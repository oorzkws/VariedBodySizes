...
This mod adds two new settings to rimworld. The first allows you to individually set a customized size range for all pawns and animal sizes. This change is strictly cosmetic and alters no other statistics of the pawn or animal.

The second, and only if you chose to enable it, enables functional changes as well. By enabling "also modify body size stat" in the mod config you will also alter the "body size" stat. This affects things like calculating meat amount, leather amount, carry capacity and various other stats. It can also be used in other mods for all sorts of reasons. Basically since this setting modifies the body size stat, it will be included in those types of calculations. 

Enabling this setting should not change things like health, melee range, melee damage, or movement speed.
...
-  Also all size changes to also alter the "body size" stat
